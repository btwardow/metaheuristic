...
pso_result<-psoptim(init_weights, predict_eval, gr=NULL, stock.week.avg,
...
pso_result$par #best weights
pso_result$value #minimal cost(eval) function
```
...
```{r}
pso_predictions <- all_week_predictions(pso_result$par)
plot(pso_predictions,type='o',col='red',
     ylim=c(min(pso_predictions),max(pso_predictions)),xlab="Week",ylab="Avg price(PLN)")
